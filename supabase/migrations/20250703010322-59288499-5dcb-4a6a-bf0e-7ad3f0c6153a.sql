-- Phase 1: Database optimizations for better performance (corrected)
...
ON public.notes_v2 USING gin(tags) 
WHERE user_id IS NOT NULL AND tags IS NOT NULL;

-- Add text search index
CREATE INDEX IF NOT EXISTS idx_notes_v2_text_search 
ON public.notes_v2 USING gin(to_tsvector('english', title || ' ' || content));
