...
const SmartDocumentAnalyzer: React.FC = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length !== files.length) {
      alert('Some files were skipped. Only .txt, .md, .pdf, .doc, .docx files are supported.');
    }
  ...
  const handleAnalyze = async () => {
    ...
    if (results.length > 0) setCurrentAnalysis(results[0]);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-4">Supports PDF, TXT, MD, DOC, and DOCX files</p>
            {selectedFiles.length > 0 && (
              <div className="flex flex-wrap gap-2 justify-center">
                {selectedFiles.map((file, index) => (
                  <Badge key={`${file.name}-${index}`} variant="secondary" className="flex items-center gap-1">
      ...
      {/* Results Section */}
      {analysisResults.length > 0 && currentAnalysis && (
            ...
            <CardContent>
              <Tabs defaultValue="summary" className="w-full">
                ...
                <TabsContent value="summary" className="space-y-4">
                  ...
                  <p className="text-muted-foreground leading-relaxed">{currentAnalysis.summary}</p>
                  <h4 className="font-semibold mb-2 flex items-center gap-2">
